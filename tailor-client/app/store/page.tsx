...
const boutiques = [
  { name: "House of Myraa", place: "Indiranagar", image: "/outfits/churidharimage.png", rating: "4.9" },
  { name: "Label Ananya", place: "Koramangala", image: "/outfits/kurthaimage.jpg", rating: "4.8" },
  { name: "The Silk Room", place: "Jayanagar", image: "/outfits/blouseimage.jpg", rating: "4.7" },
  { name: "Stitch & Sage", place: "HSR Layout", image: "/outfits/pantimage.jpg", rating: "4.9" },
];

const collections = [
  { title: "Wedding Edit", count: "32 boutiques", image: "/outfits/churidharimage.png" },
  { title: "Festive Favourites", count: "28 boutiques", image: "/outfits/kurthaimage.jpg" },
  { title: "Summer Styles", count: "18 boutiques", image: "/outfits/shirtimage.jpg" },
  { title: "Minimal Muse", count: "22 boutiques", image: "/outfits/blouseimage.jpg" },
];
      ...
      <div className="mx-auto flex max-w-[1500px]">
        <aside className="sticky top-0 hidden h-screen w-64 shrink-0 border-r border-emerald-100/80 bg-white/72 px-6 py-8 shadow-2xl shadow-emerald-950/5 backdrop-blur-xl lg:block">
          ...
          <div className="mt-10 rounded-lg border border-emerald-100 bg-white/80 p-5 shadow-xl shadow-emerald-950/5">
            <div className="relative mb-4 h-36 overflow-hidden rounded-md bg-gradient-to-br from-white to-emerald-50">
              <Image src="/outfits/blouseimage.jpg" alt="" fill className="object-contain p-4" sizes="220px" />
            </div>
            <p className="text-xl font-bold">Custom stitching made for you</p>
            <Link href="/tailor" className="mt-5 inline-flex rounded-md bg-slate-950 px-5 py-3 text-sm font-bold text-white">
              Management
            </Link>
          </div>
        </aside>
        ...
        <section className="min-w-0 flex-1 px-4 pb-24 pt-5 sm:px-6 lg:px-10">
          <header className="mb-7 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex min-h-14 flex-1 items-center gap-3 rounded-md bg-white/90 px-5 shadow-xl shadow-emerald-950/5 ring-1 ring-emerald-100 backdrop-blur md:max-w-xl">
          ...
          <section className="relative min-h-[390px] overflow-hidden rounded-xl border border-white/20 bg-slate-950 p-8 text-white shadow-2xl shadow-emerald-950/20 md:p-12">
            <Image src="/outfits/churidharimage.png" alt="" fill priority className="object-contain object-right opacity-78 drop-shadow-2xl" sizes="100vw" />
            ...
            <div className="relative max-w-xl">
              ...
              <button className="mt-7 rounded-md bg-white px-6 py-3 text-sm font-bold text-black shadow-xl shadow-black/20">Explore Collections</button>
              <div className="mt-8 grid max-w-lg grid-cols-3 gap-3">
                {["32 Collections", "4.8 Avg Rating", "Within 5 km"].map((item) => (
                  <div key={item} className="rounded-md border border-white/15 bg-white/10 px-3 py-3 text-xs font-semibold text-emerald-50 backdrop-blur">
                    {item}
                  </div>
                ))}
              </div>
          ...
          <section className="mt-6 grid grid-cols-3 gap-3 sm:grid-cols-6">
            {categories.map((item) => {
              const Icon = item.icon;
              return (
                <button key={item.label} className="flex min-h-24 flex-col items-center justify-center gap-2 rounded-lg bg-white/90 text-sm font-semibold text-slate-700 shadow-xl shadow-emerald-950/5 ring-1 ring-emerald-100 backdrop-blur">
          ...
          <section className="mt-8">
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              {boutiques.map((item) => (
                <article key={item.name} className="relative min-h-72 overflow-hidden rounded-lg bg-slate-100 shadow-2xl shadow-emerald-950/10 ring-1 ring-white/70">
                  <Image src={item.image} alt="" fill className="object-cover" sizes="(max-width: 768px) 50vw, 22vw" />
          ...
          <section className="mt-8">
            ...
            <div className="grid gap-4 md:grid-cols-4">
              {collections.map((item) => (
                <article key={item.title} className="relative min-h-40 overflow-hidden rounded-lg bg-white/92 p-5 shadow-xl shadow-emerald-950/5 ring-1 ring-emerald-100 backdrop-blur">
                  <div className="relative z-10">
                    <h3 className="font-PlayfairDisplay text-xl">{item.title}</h3>
                    <p className="mt-1 text-sm text-slate-500">{item.count}</p>
                  </div>
                  <Image src={item.image} alt="" width={110} height={140} className="absolute bottom-0 right-1 h-32 w-24 object-contain" />
                </article>
              ))}
            </div>
          </section>
          ...
          <section className="mt-8 grid gap-4 lg:grid-cols-[1fr_320px]">
            <div>
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                {boutiques.slice(0, 3).map((item, index) => (
                  <article key={item.name} className="overflow-hidden rounded-lg bg-white/92 shadow-xl shadow-emerald-950/5 ring-1 ring-emerald-100 backdrop-blur">
                    <div className="relative h-32 bg-emerald-50">
                      <Image src={item.image} alt="" fill className="object-contain p-3" sizes="220px" />
                      <span className="absolute left-3 top-3 rounded-md bg-black/70 px-2 py-1 text-xs font-bold text-white">{(index + 2.1).toFixed(1)} km</span>
                    </div>
                    <div className="p-4">
                      <h3 className="font-bold">{item.name}</h3>
            ...
            <div className="relative min-h-72 overflow-hidden rounded-lg bg-emerald-100 p-5 shadow-xl shadow-emerald-950/10 ring-1 ring-white/80">
              ...
              <button className="absolute bottom-5 left-5 rounded-md bg-white px-5 py-3 text-sm font-bold shadow">View all on map</button>
              <button className="absolute bottom-5 right-5 rounded-md bg-emerald-700 p-4 text-white" aria-label="Locate"><Navigation size={20} /></button>
            </div>
          </section>
        </section>
      </div>

      <nav className="fixed bottom-4 left-1/2 z-20 flex w-[calc(100%-2rem)] max-w-xl -translate-x-1/2 items-center justify-around rounded-lg bg-white px-5 py-3 shadow-2xl ring-1 ring-emerald-100 lg:hidden">
        {[Home, Search, Plus, CalendarDays, User].map((Icon, index) => (
          <button key={index} className={`${index === 2 ? "bg-emerald-600 text-white" : "text-slate-700"} rounded-md p-3`} aria-label={`Store action ${index + 1}`}>
            <Icon size={22} />
          </button>
        ))}
      </nav>
